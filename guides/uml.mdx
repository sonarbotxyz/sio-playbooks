...
description: "Diagrammes de cas d utilisation, classes, sequence, activite, etats, deploiement"
...
1. [1. Introduction a UML](#1-introduction-a-uml)
2. [2. Diagramme de cas d'utilisation (Use Case)](#2-diagramme-de-cas-dutilisation-use-case)
3. [3. Diagramme de classes](#3-diagramme-de-classes)
4. [4. Diagramme de sequence](#4-diagramme-de-sequence)
5. [5. Diagramme d'activite](#5-diagramme-dactivite)
6. [6. Diagramme d'etat (bases)](#6-diagramme-detat-bases)
7. [7. De UML au code et du code a UML](#7-de-uml-au-code-et-du-code-a-uml)
8. [8. Methodologie d'examen](#8-methodologie-dexamen)
9. [9. Exercices d'examen corriges](#9-exercices-dexamen-corriges)
10. [Resume final -- Aide-memoire express](#resume-final-aide-memoire-express)
...
> PIEGE EXAMEN : On peut vous demander de citer les grandes familles de diagrammes UML
> (structure vs comportement). Ne les confondez pas. Le diagramme de classes est
> STRUCTUREL (statique). Le diagramme de sequence est COMPORTEMENTAL (dynamique).
...
> PIEGE EXAMEN : Le sens des fleches est INVERSE entre include et extend.
> - Include : la fleche part du cas principal VERS le cas inclus
> - Extend : la fleche part du cas optionnel VERS le cas principal
> Ne les inversez JAMAIS. C'est l'erreur numero 1 aux examens.
...
> PIEGE EXAMEN : Le signe # signifie "protected", PAS "prive".
> Le prive c'est le signe - (tiret/moins).
...
> PIEGE EXAMEN : Les cardinalites se lisent "a l'envers". La cardinalite est placee
> du cote de la classe CIBLE, pas de la classe SOURCE. C'est l'erreur la plus
> frequente. Relisez toujours : "Un [classe gauche] a [cardinalite droite] [classe droite]".
...
> PIEGE EXAMEN : Le losange est TOUJOURS du cote du TOUT (le conteneur),
> jamais du cote de la partie. Facture <*>---- LigneFacture, pas l'inverse.
...
> Note : En C# et Java modernes, les interfaces peuvent avoir des methodes par defaut,
> mais au BTS SIO on considere le modele classique : interface = contrat pur.
...
> PIEGE EXAMEN : En Merise, les cardinalites sont du cote de l'entite concernee.
> En UML, les cardinalites sont du cote OPPOSE (pres de la classe cible).
> Exemple : Si en Merise on a Client (1,1) --- passe --- (0,n) Commande
> En UML : Client 1 ---------- 0..* Commande
> (le 1 est pres de Client car une commande a 1 client,
>  le 0..* est pres de Commande car un client a 0..* commandes)
>
> ATTENTION : la formulation ci-dessus peut preter a confusion. Soyons precis :
> En Merise, (1,1) cote Client signifie "un client passe au minimum 1, au maximum 1 commande" -- NON.
> En fait en Merise, les cardinalites se lisent : "une occurrence de cette entite participe
> [min, max] fois a cette association".
> Donc Client (1,1) passe Commande signifie : un client passe entre 1 et 1 commande.
> Et Commande (0,n) signifie : une commande... non, c'est l'inverse.
>
> Reprenons clairement :
> En Merise : Client --(0,n)-- passe --(1,1)-- Commande
> Se lit : Un Client passe entre 0 et n Commandes. Une Commande est passee par 1 et 1 seul Client.
> En UML : Client 0..* ---------- 1 Commande
> Le 0..* est PRES de Commande (un client a 0..* commandes).
> Le 1 est PRES de Client (une commande a 1 client).
>
> En resume : les cardinalites Merise "migrent" du meme cote en UML mais avec la lecture inversee.
> La cardinalite Merise (0,n) cote Client donne 0..* du cote de Commande en UML.
...
> Note : En composition, le conteneur cree et detruit ses parties.
> En agregation, les parties sont passees de l'exterieur (injection).
